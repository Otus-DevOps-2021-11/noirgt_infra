...
## Задание 1:
- Настроен Vagrant и выполнен провижинг ролей Ansible;
- Протестированы роли при помощи Molecule и Testinfra;
- Provisiong в Packer теперь работает через Ansible на основе ролей;
- Написан тест к роли db для проверки того, что DB слушает по нужному порту (27017);
## Задание с ⭐:
> Сделано не полностью!
> Подключить TravisCI для созданного репозитория с ролью db для автоматического прогона тестов в GCE сделать не смог,
> так как физически не обладаю ресурсами в GCE.
- Вынесена роль db в отдельный репозиторий (`https://github.com/noirgt/ansible-roles.git`);
- Удалена роль из репозитория infra и сделать подключение роли через requirements.yml обоих окружений.
